...
# 本地代理软件未运行/端口变更时，继承到的代理变量或 Windows 注册表系统代理会让
# OpenAI SDK 的连接被拒或超时 (WinError 10061 / TLS handshake timeout)，导致所有
# 智能体节点抛异常并回退到 fallback 文案。LLM 端点 (maas.bit.edu.cn /
# api.siliconflow.com) 均可直连：启动时清除代理变量，并用 NO_PROXY=* 让 httpx
# 同时忽略注册表系统代理，确保 LLM 流量永远直连。
...
def ensure_langchain_debug_compat() -> None:
    """Provide legacy global attrs for mixed langchain/langchain_core installs.

    langchain-core 0.3.x's ``globals.get_verbose()/get_debug()/get_llm_cache()``
    read ``langchain.verbose``/``langchain.debug``/``langchain.llm_cache`` when the
    ``langchain`` umbrella package is present. langchain 1.x removed those attrs,
    so instantiating any ChatModel raises ``AttributeError``. Backfill them here.
    """
    try:
    ...
    for attr, default in (("debug", False), ("verbose", False), ("llm_cache", None)):
        if not hasattr(langchain_module, attr):
            setattr(langchain_module, attr, default)
