...
def _build_agraph_payload(records: List[Dict[str, Any]]) -> Tuple[List[Any], List[Any], Dict[str, Dict[str, Any]]]:
    ...
    return list(node_map.values()), list(edge_map.values()), node_details
...
def _render_hypergraph_dashboard() -> None:
    ...
    try:
        ...
        st.warning("请检查 Neo4j 连接配置与数据库状态。")
        return

    if not records:
        st.warning("Neo4j 当前无可视化关系数据。")
        return
    ...
    try:
        nodes, edges, node_details = _build_agraph_payload(records)
