...
def _is_tool_activity_message(message: Any) -> bool:
    """决策 2：识别需要隐藏的图谱检索痕迹（工具回执与 AI 的工具调用请求）。"""
...
def _extract_final_assistant_message(graph_messages: List[Any]) -> Any | None:
    """提取最终文字回答：跳过工具回执与带 tool_calls 的中间 AI 消息。"""
    for message in reversed(graph_messages):
        if _is_tool_activity_message(message):
            continue
        if _message_role(message) == "assistant" and _message_content(message):
            return message
    return None
...
def render_student_page() -> None:
    ...
    with chat_container:
        for msg in st.session_state.messages:
            # 决策 2：隐藏图谱检索返回的原始 JSON 与 AI 的工具调用请求
            if _is_tool_activity_message(msg):
                continue
        ...
        with chat_container:
            with st.chat_message("assistant"):
                with st.spinner("正在思考，请稍候..."):
                    ...
                    # 决策 2：若本轮出现工具活动，用一条优雅的 UI 提示替代原始检索痕迹
                    if any(_is_tool_activity_message(message) for message in graph_messages):
                        st.markdown("*🧠 正在调阅 Neo4j 图谱数据...*")
                    assistant_message = _extract_final_assistant_message(graph_messages)
                    active_agent = final_state.get("active_agent", "未知智能体")
                    st.markdown(f"**智能体**: {active_agent} | **模型**: {get_llm_config()['model']}")
